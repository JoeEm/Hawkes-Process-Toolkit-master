...
muest = model.mu;
D = size(Aest, 1);
model.g = abs(randn(size(model.landmark)));
model.dt = 0.1;
    ...
    for n = 1:alg.inner 
        ...
        for c = 1:length(Seqs)
            ...
            for i = 1:Nc
                ...
                ui = Event(i);
            %dbstop in Learning_MLE_ODE at 70;
                AmatA(ui,:,:) = AmatA(ui,:,:)+...
...
%         end    
    ...
    CM = CM./model.g;
    gest = model.g;
    %dbstop in Learning_MLE_ODE at 157
    alg.inner_g = length(size(model.g,1))-1;
    for n = 1:alg.inner_g
        for m = 1:size(model.g, 1)
            switch m
                ...
                case size(model.g, 1)
                    ...
                    c = -DM(m,:) * (model.dt^2);
                otherwise
                    %dbstop in Learning_MLE_ODE at 170;
                    a = 4*alg.alpha + CM(m,:) * (model.dt^2);
